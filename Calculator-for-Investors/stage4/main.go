...
func getFinancialData() (ebitda, sales, netProfit, marketPrice,
	netDebt, assets, equity, cashEquivalents, liabilities float64) {
...
func getCompanyData() (ticker, name, sector string) {
...
type CompanyIndicator struct {
	Ticker string
	Value  float64
}

func sortAndPrintTopTenIndicators(companyIndicators []CompanyIndicator, funcKey string) {
	sort.Slice(companyIndicators, func(i, j int) bool {
		return companyIndicators[i].Value > companyIndicators[j].Value
	})

	fmt.Printf("TICKER %s\n", funcKey)
	for i, ci := range companyIndicators {
		if i >= 10 {
			break
		}
		trimmedValue := strings.TrimRight(fmt.Sprintf("%.2f", ci.Value), "0")
		fmt.Printf("%s %s\n", ci.Ticker, trimmedValue)
	}
}

func createCompanyIndicators(financials []Financial,
	funcMap map[string]func(*Financial) float64, funcKey string) []CompanyIndicator {
	var companyIndicators []CompanyIndicator
	for _, financial := range financials {
		value := funcMap[funcKey](&financial)
		companyIndicators = append(companyIndicators, CompanyIndicator{
...
func calculateTopTen(db *gorm.DB, funcKey string) {
	var financials []Financial
	db.Find(&financials)

	var funcMap = map[string]func(*Financial) float64{
		"ND/EBITDA": (*Financial).calculateNDEBITDA,
		"ROE":       (*Financial).calculateROE,
		"ROA":       (*Financial).calculateROA,
	}

	companyIndicators := createCompanyIndicators(financials, funcMap, funcKey)

	sortAndPrintTopTenIndicators(companyIndicators, funcKey)
}

func handleTopTenOption(option string, db *gorm.DB) {
	switch option {
	case "0":
		fmt.Printf("\n")
	case "1":
		calculateTopTen(db, "ND/EBITDA")
	case "2":
		calculateTopTen(db, "ROE")
	case "3":
		calculateTopTen(db, "ROA")
	default:
		fmt.Print(invalidOptionMsg)
	}
}
...
func processUserInput(db *gorm.DB) {
	for {
		...
		case "2":
			...
			handleTopTenOption(topTenOption, db)
		default:
			fmt.Print(invalidOptionMsg)
		}
	}
